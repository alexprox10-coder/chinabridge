...
export async function POST(req: NextRequest) {
  ...
  try {
    // Use tagged template literals — sql.unsafe() does NOT execute DDL on Vercel
    await sql`
  ...
  const indexDefs = [
    ["outbound_leads_tenant_idx",  "tenant_id"],
    ["outbound_leads_stage_idx",   "stage"],
    ["outbound_leads_country_idx", "country"],
    ["outbound_leads_vertical_idx","vertical"],
    ["outbound_leads_dedup_idx",   "dedup_hash"],
  ] as const;

  for (const [idxName, col] of indexDefs) {
    try {
      // Can't interpolate identifiers in tagged templates — use unsafe() for indexes only
      await sql.unsafe(`CREATE INDEX IF NOT EXISTS ${idxName} ON outbound_leads (${col})`);
      results.push(`OK: ${idxName}`);
    } catch (err: unknown) {
      results.push(`ERR ${idxName}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  try {
    ...
    `;
    results.push(`OK: table_exists=${(check as {count: string}[])[0]?.count ?? '?'}`);
  } catch (err: unknown) {
    results.push(`ERR verify: ${err instanceof Error ? err.message : String(err)}`);
  }

  try {
    const rows = await sql`SELECT COUNT(*) as count FROM outbound_leads`;
    results.push(`OK: rows=${(rows as {count: string}[])[0]?.count ?? '?'}`);
  } catch (err: unknown) {
