...
async function vkGet(path: string, token: string): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`VK API ${path} → ${res.status}: ${err}`);
  }
  return res.json();
}
...
async function runSync(): Promise<NextResponse> {
  ...
  try {
    // Получаем список lead-форм
    const formsData = await vkGet('/lead_forms/', token) as { items?: VkLeadForm[] };
    ...
    for (const form of forms) {
      try {
        const leadsData = await vkGet(`/lead_forms/${form.id}/leads/`, token) as { items?: VkLead[] };
