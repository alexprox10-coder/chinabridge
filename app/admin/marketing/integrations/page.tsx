    ...
    <>
      ...
      <div className="px-4 lg:px-8 py-6 max-w-4xl space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className={`bg-slate-900 border rounded-2xl p-5 flex flex-col gap-3 ${
            vkStatus?.connected ? "border-blue-700/40" : "border-slate-800"
          }`}>
            <div className="flex items-center justify-between">
              ...
              {vkLoading ? (
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-800 text-slate-500 animate-pulse">
                  Загрузка…
                </span>
              ) : vkStatus?.connected ? (
            ...
            {vkStatus?.connected ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={handleSync}
                    disabled={syncing}
                    className="flex-1 text-xs py-2 bg-blue-900/30 hover:bg-blue-900/50 text-blue-300 border border-blue-700/30 rounded-xl transition-colors disabled:opacity-50"
                  >
                    {syncing ? "Синхронизация…" : "Синхронизировать лиды"}
                  </button>
                  <a
                    href="/api/vk-ads/auth"
                    className="text-xs py-2 px-3 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-xl transition-colors"
                  >
                    Переподключить
                  </a>
                </div>
              ...
              <a
                href="/api/vk-ads/auth"
                className="w-full text-xs py-2 bg-blue-900/30 hover:bg-blue-900/50 text-blue-300 border border-blue-700/30 rounded-xl transition-colors text-center"
              >
          ...
          {STATIC_INTEGRATIONS.map((intg) => (
            <div
              ...
              className={`bg-slate-900 border rounded-2xl p-5 flex flex-col gap-3 ${
                intg.status === "demo" ? "border-amber-700/30" : "border-slate-800"
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
