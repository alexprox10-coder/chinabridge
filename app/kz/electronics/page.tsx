...
export default function KzElectronicsPage() {
    ...
    try {
      ...
      await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          name:       form.name,
          phone:      form.phone,
          product:    form.product || "Электроника из Китая",
          source:     "kz_electronics",
          utm_source: params.get("utm_source") ?? "vk",
          utm_campaign: params.get("utm_campaign") ?? "kz_electronics",
          country_destination: "Kazakhstan",
          city_destination: "Алматы",
          comment: `KZ электроника. Товар: ${form.product}`,
        }),
      });
      setStatus("ok");
    } catch {
      ...
      <main className="min-h-screen bg-[#050e1d] text-white">
        <section className="pt-20 pb-12 px-4">
          <div className="max-w-2xl mx-auto text-center">
            <p className="text-xs font-semibold text-[#00A86B] uppercase tracking-widest mb-3">Казахстан · Электроника из Китая</p>
            <h1 className="text-3xl sm:text-4xl font-bold leading-tight mb-4">
              Электроника из Китая<br className="hidden sm:block" /> в Казахстан
            </h1>
            <p className="text-[#8899aa] text-base leading-relaxed max-w-lg mx-auto mb-8">
              Найдём поставщика или привезём от вашего.<br />
              Рассчитайте стоимость поставки за 15 минут.
            </p>

            <div className="flex flex-wrap justify-center gap-6 mb-10 text-sm">
              {[
                { n: "17%", label: "продаж Kaspi — электроника" },
        ...
        <section className="pb-8 px-4">
          <div className="max-w-2xl mx-auto">
            <div className="flex flex-wrap justify-center gap-2 mb-8">
              {["Смартфоны", "Наушники / TWS", "Умные часы", "Планшеты", "Зарядные устройства", "Павербанки", "LED-лампы", "Умный дом"].map(tag => (
          ...
          <div className="max-w-lg mx-auto">
            {status === "ok" ? (
              ...
              <form onSubmit={submit} className="rounded-2xl border border-[#1a3a5c] bg-[#0B1F3A]/60 p-6 sm:p-8 space-y-4">
                ...
                <p className="text-[#8899aa] text-sm mb-4">Укажите товар или ссылку с 1688/Alibaba</p>
                ...
                <div>
                  ...
                  <input
                    required
                    type="text"
                    placeholder="Например: TWS-наушники ANC, 200 шт, ссылка 1688..."
                    value={form.product}
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center text-sm">
              {[
                { icon: "🔍", title: "Поиск поставщика", desc: "Находим на 1688, Alibaba, заводах" },
            ...
            <div className="mt-6 text-center">
              <Link href="/ai-calculator" className="text-[#00A86B] text-sm hover:underline">
                Рассчитать юнит-экономику в AI-калькуляторе →
              </Link>
            </div>
          </div>
        </section>
