# Graph Report - chinabridge  (2026-09-21)

## Corpus Check
- 837 files · ~1,084,150 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4833 nodes · 8867 edges · 345 communities (267 shown, 78 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 40 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f1372664`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- content/db.ts
- marketing/db.ts
- partner-portal/api.ts
- FinanceDashboard.tsx
- content/data.ts
- analytics/data.ts
- OperationsDashboard.tsx
- strategy/data.ts
- MarketingDashboard.tsx
- clusters.ts
- getLLMConfig
- ContentPageClient.tsx
- generator.ts
- CategoryPage.tsx
- import-leads/types.ts
- rate-engine/db.ts
- ceo/report/route.ts
- createLead
- eis_collector_v2.py
- LeadDetail.tsx
- rate-engine/types.ts
- isAuthorized
- tochka/client.ts
- outbound/import/route.ts
- CeoDashboard.tsx
- signup/route.ts
- market-intelligence/db.ts
- OnboardingWizard.tsx
- intelligence/page.tsx
- seed-runner.ts
- SalesDashboard.tsx
- document.tsx
- compilerOptions
- deal-intelligence/index.ts
- logIntakeEvent
- rate-calculator.ts
- ai/client.ts
- client-portal/types.ts
- CTASection.tsx
- AiOsDashboard.tsx
- documents/types.ts
- ai-cto/index.ts
- blog/[slug]/page.tsx
- outbound/scoring.ts
- enrich-v2/route.ts
- devDependencies
- market-intelligence/types.ts
- payments/create/route.ts
- MarketingPageClient.tsx
- app/page.tsx
- AIEconomicsFunnel.tsx
- client/layout.tsx
- Breadcrumbs.tsx
- import-audit/page.tsx
- dependencies
- api/leads/route.ts
- lead-finder/index.ts
- ai/types.ts
- validators.ts
- free/page.tsx
- client-portal/auth.ts
- market-radar/index.ts
- partners/db.ts
- vk-intent/pipeline.ts
- intelligence/client.ts
- vk-scraper/route.ts
- TgMonitorClient.tsx
- intake/page.tsx
- CurrencyRates.tsx
- tender-intelligence/route.ts
- client/calculator/page.tsx
- DealIntelligenceClient.tsx
- setup/route.ts
- UnifiedLeadsDashboard.tsx
- pricing/page.tsx
- admin/calculations/page.tsx
- KnowledgeUI.tsx
- Footer.tsx
- eis_collector_beget.py
- enrich.ts
- free-guide.tsx
- url-parser.ts
- eis_collector.py
- ai-cto/db.ts
- pricing-engine.ts
- SalesDashboardClient.tsx
- preview/route.ts
- getSql
- collect/route.ts
- sources/route.ts
- dev-reset/route.ts
- outreach-leads/page.tsx
- departments.ts
- CalcRightPanel.tsx
- SeoClustersClient.tsx
- memory.ts
- app/services/page.tsx
- history/route.ts
- import-leads/crm.ts
- calculator/types.ts
- WbSellersClient.tsx
- NicheLeadPage.tsx
- sales/data.ts
- telegram/webhook/route.ts
- tenders/page.tsx
- SupplierFinderUI.tsx
- CreateWizard.tsx
- [id]/offer/route.ts
- placements/page.tsx
- fix/route.ts
- pdftest/route.ts
- ensureIntelligenceSchema
- AuditClient.tsx
- migrate-pg.cjs
- ai-company/page.tsx
- SalesCompaniesClient.tsx
- scripts
- CtoDashboard.tsx
- product-finder/page.tsx
- drizzle-orm
- ImportLead
- articles/route.ts
- outbound/page.tsx
- presentation/page.tsx
- finance/api.ts
- analytics/index.ts
- Weekly Priorities
- CHINABRIDGE — AI OPERATING SYSTEM
- client/calculations/page.tsx
- market-watch/route.ts
- PersonalOfferWidget.tsx
- settings/marketing/page.tsx
- Marketplace.tsx
- tenant-storage.ts
- Russia Import Rules
- economics/route.ts
- KPIs by Category
- finance/settings/page.tsx
- getAllLeads
- contact-alert/route.ts
- HhLeadsClient.tsx
- client-portal/api.ts
- seed-db.ts
- crm/client.ts
- PartnersDashboard.tsx
- SignupForm.tsx
- isAdminOnly
- vk-ads/sync/route.ts
- multitenant/types.ts
- rates/sync/route.ts
- wildberries-margin-calculator/page.tsx
- campaigns/page.tsx
- getSession
- content-publish/route.ts
- free/route.ts
- facts/route.ts
- demo/documents/page.tsx
- approve/route.ts
- AiSettings.tsx
- BillingCenter.tsx
- registrations/page.tsx
- database.ts
- schema.ts
- calculator.ts
- lid-webhook/route.ts
- middleware.ts
- vercel.json
- dtQuery
- VkIntentClient.tsx
- chat/page.tsx
- tg-scraper/route.ts
- checks/crm.ts
- client/page.tsx
- crm/page.tsx
- debug-secret.mjs
- tenant-auth.ts
- checks/api.ts
- store.ts
- white-import-wb-ozon/page.tsx
- Architecture Decision Records (ADR)
- ChatWidget.tsx
- trigger-enrich/route.ts
- Intelligence Agent — Context
- wb-sellers/route.ts
- migrate/route.ts
- plans/page.tsx
- Sales Agent System Prompt
- clients/route.ts
- ai/route.ts
- marketing-ai/route.ts
- client/login/page.tsx
- migrate-outbound.mjs
- init/route.ts
- demo/finance/page.tsx
- tripwire/route.ts
- Kazakhstan Import Rules
- backup/page.tsx
- query-approved.mjs
- cleanup/leads/route.ts
- test/route.ts
- CalcLeftPanel.tsx
- export/route.ts
- import/route.ts
- complete/route.ts
- Customer Success Agent — Context
- demo-request/route.ts
- demo/calculator/page.tsx
- demo/page.tsx
- partner/page.tsx
- icon.tsx
- BackLink.tsx
- SaaSPlatformBlock.tsx
- import-leads/page.tsx
- admin/layout.tsx
- market-intelligence/leads/page.tsx
- client/auth/google/route.ts
- partner/auth/google/route.ts
- translate/route.ts
- vk-ads/callback/route.ts
- client/profile/page.tsx
- demo/layout.tsx
- AiCompanyOsBlock.tsx
- Process.tsx
- validation.ts
- calc-searches/route.ts
- lucide-react
- @neondatabase/serverless
- next.config.ts
- postgres
- react-hook-form
- @supabase/supabase-js
- tailwind-merge
- postcss.config.mjs
- tailwind.config.ts
- ai-funnel/route.ts
- Sales Agent — Context
- SEO Agent — Context
- VkAdsDashboard.tsx
- follow-up-reminder/route.ts
- ChinaBridge — Business Model
- MarketNewsBar.tsx
- WarehouseGallery.tsx
- cbr/route.ts
- Marketplace Commission Rules
- check-outbound.cjs
- log-search/route.ts
- ChinaBridge — Business Knowledge
- Strategy Directory
- reset-rate-limit/route.ts
- reset/route.ts
- lib/finance/types.ts
- currency/route.ts
- import-consultant.ts
- queries/page.tsx
- CeoReportWidget.tsx
- telegram-capture/route.ts
- check-paid/route.ts
- OnboardingSteps.tsx
- kaspi-china/layout.tsx
- reset-rate-limit.mjs
- create-calculator-leads.mjs
- create-calculator-leads-pg.mjs
- create-calculator-leads-postgres.mjs
- checks/telegram.ts
- faq/page.tsx
- update-rates.mjs
- support-webhook/route.ts
- company-enrich/route.ts
- clients-debug/route.ts
- knowledge/chat/route.ts
- TochkaLinksSection.tsx
- install.sh
- calc-verify/route.ts
- intake/route.ts
- leads-products/route.ts
- create-tripwire/route.ts
- ai-chat/page.tsx
- set-pro/route.ts
- calculator-lead/route.ts
- FAQSection.tsx
- calculator-success/page.tsx
- marketplace-rates/route.ts
- mode-select/route.ts
- create-outbound-table.cjs
- clear-pro/route.ts
- thank-you/page.tsx
- seo.ts
- migrate-outbound.cjs
- outbound/init/route.ts
- trade-map/page.tsx
- requisites/page.tsx
- tg-click/route.ts
- trackGAEvent
- funnel-report/route.ts
- outbound/debug/route.ts
- update-rates/route.ts
- ai-chat/route.ts
- trigger-migrate.mjs
- product-finder/route.ts
- indexnow/route.ts
- RelatedContent.tsx
- faq.ts
- outbound/leads/route.ts
- calc-funnel-tracker.ts
- AdminNav.tsx
- outbound/stats/route.ts
- payments.ts
- security.ts
- rates/route.ts
- china-blagoveshchensk/page.tsx
- runOutboundMigrations
- ai-providers.ts
- china-khabarovsk/page.tsx
- funnel/page.tsx
- activate-anon/route.ts
- notifications/route.ts
- parser-club-webhook/route.ts
- funnel-event/route.ts
- app/layout.tsx
- china-moscow/page.tsx
- calc-step/route.ts
- supplier-search/page.tsx
- context/route.ts
- pay-supplier-china/page.tsx
- calc-funnel/page.tsx
- calc-funnel/route.ts
- calc-claim/route.ts
- getLeads

## God Nodes (most connected - your core abstractions)
1. `isAuthorized()` - 109 edges
2. `AdminNav()` - 62 edges
3. `getDb()` - 40 edges
4. `createLead()` - 39 edges
5. `getTenantId()` - 35 edges
6. `getSession()` - 34 edges
7. `getLLMConfig()` - 28 edges
8. `getLeads()` - 25 edges
9. `Breadcrumbs()` - 25 edges
10. `trackGAEvent()` - 24 edges

## Surprising Connections (you probably didn't know these)
- `BillingPage()` --calls--> `getTenantById()`  [EXTRACTED]
  app/admin/settings/billing/page.tsx → lib/multitenant/store.ts
- `GET()` --calls--> `getStats()`  [EXTRACTED]
  app/api/intelligence/init/route.ts → lib/intelligence/client.ts
- `POST()` --calls--> `upsertFact()`  [EXTRACTED]
  app/api/intelligence/facts/route.ts → lib/intelligence/client.ts
- `FinanceSettingsPage()` --calls--> `getFinanceSettings()`  [EXTRACTED]
  app/admin/finance/settings/page.tsx → lib/finance/api.ts
- `WizardForm` --references--> `BillingPlan`  [EXTRACTED]
  app/admin/tenants/create/CreateWizard.tsx → lib/multitenant/types.ts

## Import Cycles
- None detected.

## Communities (345 total, 78 thin omitted)

### Community 0 - "content/db.ts"
Cohesion: 0.06
Nodes (86): GET(), PATCH(), POST(), requireAdmin(), runtime, DEMO_FUNNEL, GET(), requireAdmin() (+78 more)

### Community 1 - "marketing/db.ts"
Cohesion: 0.06
Nodes (85): buildDemoChannel(), buildRealDbChannel(), CrmStats, DEMO_CHANNELS, DemoChannel, GET(), getCrmStats(), getSql() (+77 more)

### Community 2 - "partner-portal/api.ts"
Cohesion: 0.05
Nodes (65): GET(), runtime, setPartnerCookie(), POST(), runtime, POST(), runtime, GET() (+57 more)

### Community 3 - "FinanceDashboard.tsx"
Cohesion: 0.06
Nodes (60): CostTab(), DirectorTab(), FinanceDashboard(), fmt(), fmtM(), ForecastTab(), OverviewTab(), priorityBadge() (+52 more)

### Community 4 - "content/data.ts"
Cohesion: 0.06
Nodes (51): CHANNEL_COLORS, ContentDashboard(), PRI, Props, SOURCE_LABEL, STATUS_STYLE, TABS, TYPE_ICONS (+43 more)

### Community 5 - "analytics/data.ts"
Cohesion: 0.07
Nodes (48): AnalyticsDashboard(), BITab(), insightIcon(), OverviewTab(), priorityBadge(), RecsTab(), statusBg(), statusColor() (+40 more)

### Community 6 - "OperationsDashboard.tsx"
Cohesion: 0.07
Nodes (48): DEAL_STATUS_COLOR, DEAL_STATUS_LABEL, DirectorTab(), DOC_STATUS_COLOR, DOC_STATUS_LABEL, DOC_TYPE_LABEL, OperationsDashboard(), OverviewTab() (+40 more)

### Community 7 - "strategy/data.ts"
Cohesion: 0.06
Nodes (42): dynamic, loadReport(), StrategyPage(), IMPACT_CFG, POTENTIAL_CFG, PRIORITY_CFG, Props, StrategyDashboard() (+34 more)

### Community 8 - "MarketingDashboard.tsx"
Cohesion: 0.07
Nodes (44): MarketingDashboard(), PLATFORM_STYLE, PRIORITY_STYLE, Props, STATUS_STYLE, TABS, dynamic, loadReport() (+36 more)

### Community 9 - "clusters.ts"
Cohesion: 0.16
Nodes (15): auth(), GET(), getDynamicKeywords(), POST(), ensureTable(), GET(), POST(), Competition (+7 more)

### Community 10 - "getLLMConfig"
Cohesion: 0.12
Nodes (19): ensureTable(), GET(), POST(), isAuthorized(), maxDuration, POST(), runtime, fallbackOffer() (+11 more)

### Community 11 - "ContentPageClient.tsx"
Cohesion: 0.09
Nodes (34): CHIP_COLORS, ContentCalendarPage(), dayKey(), parseTimes(), Post, postDate(), Schedule, WEEKDAYS (+26 more)

### Community 12 - "generator.ts"
Cohesion: 0.07
Nodes (38): dynamic, MODE_COLORS, ProposalsPage(), revalidate, STATUS_COLORS, STATUS_LABELS, POST(), runtime (+30 more)

### Community 13 - "CategoryPage.tsx"
Cohesion: 0.07
Nodes (15): metadata, metadata, metadata, metadata, metadata, metadata, metadata, metadata (+7 more)

### Community 14 - "import-leads/types.ts"
Cohesion: 0.12
Nodes (29): GET(), maxDuration, POST(), QUICK_OVERRIDES, runtime, analyzeWebsite(), orAnalyze(), scrapeWebsite() (+21 more)

### Community 15 - "rate-engine/db.ts"
Cohesion: 0.10
Nodes (21): RateBenchmark, revalidate, runtime, DELETE(), runtime, DELETE(), runtime, DELETE() (+13 more)

### Community 16 - "ceo/report/route.ts"
Cohesion: 0.11
Nodes (25): calcHealth(), GET(), maxDuration, runtime, toSummary(), generateDecisions(), toPriority(), buildFallback() (+17 more)

### Community 17 - "createLead"
Cohesion: 0.10
Nodes (22): dynamic, isAuthorized(), POST(), runtime, scoreFromReviews(), POST(), escapeNonAscii(), handlePost() (+14 more)

### Community 18 - "eis_collector_v2.py"
Cohesion: 0.15
Nodes (20): clean_html(), extract_after_title(), extract_price(), fetch(), get_total_pages(), load_state(), main(), parse_contract_card() (+12 more)

### Community 19 - "LeadDetail.tsx"
Cohesion: 0.11
Nodes (19): dynamic, revalidate, ALL_STATUSES, COLUMNS, LeadCard(), msAgo(), timeLabel(), FinanceTab() (+11 more)

### Community 20 - "rate-engine/types.ts"
Cohesion: 0.10
Nodes (21): BLANK, CARGO_TYPES, CURRENCIES, RATE_TYPES, SOURCE_BADGE, TRANSPORT_TYPES, BLANK, TRANSPORT_TYPES (+13 more)

### Community 21 - "isAuthorized"
Cohesion: 0.05
Nodes (54): dynamic, GET(), runtime, dynamic, PATCH(), runtime, dynamic, GET() (+46 more)

### Community 22 - "tochka/client.ts"
Cohesion: 0.12
Nodes (23): dynamic, GET(), PendingRow, runtime, getPriceForClient(), maxDuration, POST(), runtime (+15 more)

### Community 23 - "outbound/import/route.ts"
Cohesion: 0.20
Nodes (16): clean(), maxDuration, parseCsvOrTxt(), ParsedRow, parseXlsx(), POST(), runtime, splitCsvLine() (+8 more)

### Community 24 - "CeoDashboard.tsx"
Cohesion: 0.10
Nodes (21): buildInbox(), CeoAiTab(), CeoDashboard(), DecisionsTab(), DEPT_LINKS, DeptsTab(), fmtDate(), fmtTime() (+13 more)

### Community 25 - "signup/route.ts"
Cohesion: 0.10
Nodes (27): POST(), runtime, dynamic, GET(), runtime, POST(), runtime, ipAttempts (+19 more)

### Community 26 - "market-intelligence/db.ts"
Cohesion: 0.16
Nodes (25): buildMIComment(), dynamic, GET(), getTenantId(), maxDuration, PATCH(), POST(), runtime (+17 more)

### Community 27 - "OnboardingWizard.tsx"
Cohesion: 0.07
Nodes (17): ACTIONS, ActionState, Company, CONFETTI_COLORS, CONFETTI_ITEMS, CREATING_ITEMS, DAILY_TASKS, DEPARTMENTS (+9 more)

### Community 28 - "intelligence/page.tsx"
Cohesion: 0.07
Nodes (17): ChangeStatus, Confidence, CONFIDENCE_COLORS, DEPT_COLORS, DEPT_LABELS, IMPACT_LABELS, IMPACT_STYLES, ImpactLevel (+9 more)

### Community 29 - "seed-runner.ts"
Cohesion: 0.21
Nodes (16): getSeedRates(), SeedRate, SEED_ROUTES, SeedRoute, err(), loadEnv(), log(), main() (+8 more)

### Community 30 - "SalesDashboard.tsx"
Cohesion: 0.09
Nodes (27): dynamic, loadReport(), metadata, SalesPage(), PRIO, Priority, SalesDashboard(), Tab (+19 more)

### Community 31 - "document.tsx"
Cohesion: 0.09
Nodes (11): DARK, GREY, LIGHT_GREY, RED, styles, WHITE, CONTACTS, COOPERATION_STEPS (+3 more)

### Community 32 - "compilerOptions"
Cohesion: 0.07
Nodes (26): dom, dom.iterable, esnext, next-env.d.ts, .next/types/**/*.ts, node_modules, **/*.ts, **/*.tsx (+18 more)

### Community 33 - "deal-intelligence/index.ts"
Cohesion: 0.18
Nodes (14): dynamic, MarketIntelligencePage(), dynamic, GET(), getTenantId(), runtime, analyzeDealIntelligence(), calcForecast() (+6 more)

### Community 34 - "logIntakeEvent"
Cohesion: 0.38
Nodes (5): POST(), runtime, PATCH(), IntakeEventType, logIntakeEvent()

### Community 35 - "rate-calculator.ts"
Cohesion: 0.23
Nodes (14): listRows(), normalize(), applyRules(), calcBaseCost(), calcServiceCost(), calculateDeliveryCost(), computeChargeableWeight(), getDefaultTransportType() (+6 more)

### Community 36 - "ai/client.ts"
Cohesion: 0.18
Nodes (22): maxDuration, POST(), runtime, runConsultant(), AGENT_LABELS, runAgent(), runLogistic(), runOperator() (+14 more)

### Community 37 - "client-portal/types.ts"
Cohesion: 0.09
Nodes (16): ClientThread, DocumentsPage(), CITIES, getClientDocuments(), ClientDocument, ClientMessage, ClientOrder, ClientRole (+8 more)

### Community 38 - "CTASection.tsx"
Cohesion: 0.10
Nodes (18): CITIES, metadata, FAQ, metadata, schema, FAQ, metadata, schema (+10 more)

### Community 39 - "AiOsDashboard.tsx"
Cohesion: 0.13
Nodes (16): PRIORITY_CONFIG, Props, STATUS_CONFIG, CeoAnalysis, CeoTask, AgentInfo, CeoRecommendation, CeoReport (+8 more)

### Community 40 - "documents/types.ts"
Cohesion: 0.06
Nodes (45): CompanySettingsPage(), dynamic, dynamic, PATCH(), runtime, dynamic, fetchRows(), GET() (+37 more)

### Community 41 - "ai-cto/index.ts"
Cohesion: 0.16
Nodes (19): GET(), maxDuration, runtime, checkPage(), checkPerformance(), PAGES, buildRecommendations(), buildSection() (+11 more)

### Community 42 - "blog/[slug]/page.tsx"
Cohesion: 0.17
Nodes (14): BlogPage(), metadata, ArticlePage(), generateMetadata(), Props, renderContent(), sitemap(), ArticleCard() (+6 more)

### Community 43 - "outbound/scoring.ts"
Cohesion: 0.16
Nodes (14): ChinaMatch, ChinaMatchResult, findChinaMatch(), OR_KEY(), calculateEvidenceScore(), calculateScore(), CATEGORY_RELEVANCE, CHINA_AVAILABILITY (+6 more)

### Community 44 - "enrich-v2/route.ts"
Cohesion: 0.13
Nodes (22): assessDataQuality(), buildStructuredEvidence(), computeDqScore(), deriveCompanyId(), LeadRow, maxDuration, POST(), runtime (+14 more)

### Community 45 - "devDependencies"
Cohesion: 0.09
Nodes (23): dotenv-cli, drizzle-kit, eslint, eslint-config-next, devDependencies, dotenv-cli, drizzle-kit, eslint (+15 more)

### Community 46 - "market-intelligence/types.ts"
Cohesion: 0.11
Nodes (16): PIPE_COLORS, SOURCE_ICON, Tab, TEMP_COLORS, dynamic, DEMO_SIGNALS, RadarClient(), Tab (+8 more)

### Community 47 - "payments/create/route.ts"
Cohesion: 0.18
Nodes (19): GET(), maxDuration, runtime, maxDuration, POST(), runtime, dynamic, ensureAuditTable() (+11 more)

### Community 48 - "MarketingPageClient.tsx"
Cohesion: 0.11
Nodes (15): metadata, Analytics, Channel, fmtNum(), fmtPct(), fmtRub(), FunnelStep, KPI (+7 more)

### Community 49 - "app/page.tsx"
Cohesion: 0.07
Nodes (10): metadata, schemaOrg, Advantages, Directions, faqs, CARDS, COMPARISON_ROWS, TOPICS (+2 more)

### Community 50 - "AIEconomicsFunnel.tsx"
Cohesion: 0.07
Nodes (31): AIEconomicsFunnel(), ANALYZE_STAGES, CalcMode, CITY_CHIPS, CorrectionData, detectCountry(), EMPTY_CORRECTION, EMPTY_PRODUCT (+23 more)

### Community 51 - "client/layout.tsx"
Cohesion: 0.32
Nodes (6): ClientLayout(), getCalcPaid(), metadata, ClientNav(), NAV, SessionPayload

### Community 52 - "Breadcrumbs.tsx"
Cohesion: 0.11
Nodes (10): metadata, DUTIES, FAQ, metadata, schema, metadata, metadata, Breadcrumbs() (+2 more)

### Community 53 - "import-audit/page.tsx"
Cohesion: 0.10
Nodes (19): FulfilmentClient(), MARKETS, SERVICES, STEPS, WAREHOUSES, metadata, AUDIT_POINTS, jsonLd (+11 more)

### Community 54 - "dependencies"
Cohesion: 0.10
Nodes (21): autoprefixer, clsx, framer-motion, @hookform/resolvers, next, dependencies, autoprefixer, clsx (+13 more)

### Community 55 - "api/leads/route.ts"
Cohesion: 0.18
Nodes (16): dynamic, notifyManagerTelegram(), POST(), REQUIRED, REQUIRES_PHONE, runtime, validate(), LeadData (+8 more)

### Community 56 - "lead-finder/index.ts"
Cohesion: 0.24
Nodes (17): saveMILead(), aiScore(), Contacts, dedup(), extractContacts(), firecrawlSearch(), GOOGLE_QUERIES, heuristic() (+9 more)

### Community 57 - "ai/types.ts"
Cohesion: 0.16
Nodes (15): AgentBadge(), CONFIG, ChatInput(), Props, ChatMessage(), Props, Message, Props (+7 more)

### Community 58 - "validators.ts"
Cohesion: 0.13
Nodes (15): POST(), runtime, POST(), runtime, POST(), runtime, POST(), runtime (+7 more)

### Community 59 - "free/page.tsx"
Cohesion: 0.33
Nodes (4): benefits, faqs, jsonLd, metadata

### Community 60 - "client-portal/auth.ts"
Cohesion: 0.23
Nodes (15): GET(), notifyAdmin(), runtime, setSessionCookie(), ipAttempts, notifyAdmin(), POST(), runtime (+7 more)

### Community 61 - "market-radar/index.ts"
Cohesion: 0.21
Nodes (18): dynamic, GET(), getTenantId(), maxDuration, POST(), runtime, saveRadarSignal(), aiAnalyze() (+10 more)

### Community 62 - "partners/db.ts"
Cohesion: 0.22
Nodes (14): GET(), runtime, GET(), runtime, GET(), runtime, bootstrap(), getOrCreateLink() (+6 more)

### Community 63 - "vk-intent/pipeline.ts"
Cohesion: 0.05
Nodes (65): dynamic, GET(), maxDuration, runtime, GET(), DELETE(), dynamic, GET() (+57 more)

### Community 64 - "intelligence/client.ts"
Cohesion: 0.13
Nodes (18): CreateMarketWatchInput, FactSnapshot, MarketWatchItem, SQL, ChangeStatus, ChangeType, Confidence, CrawlFrequency (+10 more)

### Community 65 - "vk-scraper/route.ts"
Cohesion: 0.23
Nodes (12): classifyIntent(), dynamic, GET(), INTENT_KEYWORDS, maxDuration, notifyManager(), resolveGroupId(), runtime (+4 more)

### Community 66 - "TgMonitorClient.tsx"
Cohesion: 0.22
Nodes (7): dynamic, LogItem, RunResult, Tab, TG_TARGETS, TgMonitorClient(), VK_TARGETS

### Community 67 - "intake/page.tsx"
Cohesion: 0.16
Nodes (15): APPROVAL_STYLES, ApprovalStatus, EvidenceItem, IntakeDashboard(), IntakeLead, INTENT_LABELS, KPI, leadAge() (+7 more)

### Community 68 - "CurrencyRates.tsx"
Cohesion: 0.32
Nodes (7): CbrResponse, CbrValute, CurrencyRates(), fetchRates(), fmt(), Rates, STATIC_RATES

### Community 69 - "tender-intelligence/route.ts"
Cohesion: 0.08
Nodes (62): authOk(), maxDuration, POST(), runtime, dynamic, GET(), PATCH(), authOk() (+54 more)

### Community 70 - "client/calculator/page.tsx"
Cohesion: 0.12
Nodes (15): AI_STEPS, CalculatorPage(), CalcVariant, CATEGORIES, CITIES_FROM, CITY_COUNTRIES, fmtDays(), FormData (+7 more)

### Community 71 - "DealIntelligenceClient.tsx"
Cohesion: 0.20
Nodes (5): ALERT_COLORS, DealIntelligenceClient(), RISK_COLORS, dynamic, DealScore

### Community 72 - "setup/route.ts"
Cohesion: 0.23
Nodes (12): dynamic, POST(), runtime, dynamic, maxDuration, POST(), runtime, DEMO_LEADS (+4 more)

### Community 73 - "UnifiedLeadsDashboard.tsx"
Cohesion: 0.14
Nodes (15): ACTION_BUTTONS, LeadSystem, MI_TO_UNIFIED, normalizeImport(), normalizeMI(), SOURCE_ICON, STATUS_COLORS, STATUS_LABELS (+7 more)

### Community 74 - "pricing/page.tsx"
Cohesion: 0.12
Nodes (12): BLANK, CUSTOMER_COLORS, CUSTOMER_LABELS, FilterKey, FxRate, MpRate, PricingPage(), RULE_LABELS (+4 more)

### Community 75 - "admin/calculations/page.tsx"
Cohesion: 0.38
Nodes (6): AdminCalculationsPage(), dynamic, fmtDate(), revalidate, TRANSPORT_LABELS, getAllCalculations()

### Community 76 - "KnowledgeUI.tsx"
Cohesion: 0.13
Nodes (10): metadata, ArticleFull, ArticleSummary, Category, CATEGORY_COLORS, ChatMessage, FAQ_ITEMS, KnowledgeUI() (+2 more)

### Community 77 - "Footer.tsx"
Cohesion: 0.04
Nodes (31): competitors, jsonLd, metadata, jsonLd, metadata, RATES, jsonLd, metadata (+23 more)

### Community 78 - "eis_collector_beget.py"
Cohesion: 0.33
Nodes (11): ftp_download(), ftp_list(), load_state(), main(), _num(), parse_xml(), post_batch(), relevant() (+3 more)

### Community 79 - "enrich.ts"
Cohesion: 0.22
Nodes (12): dynamic, maxDuration, POST(), runtime, enrichLead(), EnrichResult, ensureAiColumns(), extractContacts() (+4 more)

### Community 80 - "free-guide.tsx"
Cohesion: 0.20
Nodes (6): dynamic, GET(), runtime, routes, styles, tips

### Community 81 - "url-parser.ts"
Cohesion: 0.20
Nodes (15): checkRateLimit(), getIp(), maxDuration, POST(), runtime, ALLOWED, detectPlatform(), FieldConfidence (+7 more)

### Community 82 - "eis_collector.py"
Cohesion: 0.18
Nodes (18): download_ftp_file(), is_china_relevant(), list_ftp_files(), load_state(), parse_contract_xml(), Скачивает файл с FTP в dest., Парсит XML-выгрузку контрактов ЕИС, возвращает нормализованные тендеры., Быстрый фильтр до отправки на AI — экономим кредиты. (+10 more)

### Community 83 - "ai-cto/db.ts"
Cohesion: 0.31
Nodes (12): CtoPage(), dynamic, metadata, GET(), runtime, ensureCtoTable(), getCtoReportById(), getCtoReportHistory() (+4 more)

### Community 84 - "pricing-engine.ts"
Cohesion: 0.26
Nodes (12): calculateCostBreakdown(), CostEngineInput, buildReason(), ruleMatchesInput(), selectPricingRule(), CostBreakdown, PricingSelection, PricingSelectorInput (+4 more)

### Community 85 - "SalesDashboardClient.tsx"
Cohesion: 0.11
Nodes (17): dynamic, revalidate, AiFunnelData, AiScoreGroup, CrmStats, fmt(), HotCompany, NAV_TABS (+9 more)

### Community 86 - "preview/route.ts"
Cohesion: 0.36
Nodes (8): checkRateLimit(), getClientIdFromCookie(), getIp(), hasActiveSubscription(), maxDuration, POST(), runCalc(), runtime

### Community 87 - "getSql"
Cohesion: 0.18
Nodes (15): dynamic, PATCH(), runtime, dynamic, GET(), POST(), runtime, applyChange() (+7 more)

### Community 88 - "collect/route.ts"
Cohesion: 0.29
Nodes (7): CollectSource, dynamic, extractCommissionWithAI(), maxDuration, POST(), runtime, SOURCE_CONFIG

### Community 89 - "sources/route.ts"
Cohesion: 0.38
Nodes (6): dynamic, GET(), POST(), runtime, createSource(), getSources()

### Community 90 - "dev-reset/route.ts"
Cohesion: 0.67
Nodes (3): dynamic, getClientId(), POST()

### Community 91 - "outreach-leads/page.tsx"
Cohesion: 0.18
Nodes (11): APIFY_PRESETS, ApifyLead, DORK_TEMPLATES, EMPTY_LEAD, EMPTY_WB, Lead, OutreachLeadsPage(), useWorkflowRunner() (+3 more)

### Community 92 - "departments.ts"
Cohesion: 0.46
Nodes (14): agent(), buildAllDepartments(), buildAnalyticsDept(), buildClientSuccessDept(), buildContentDept(), buildFinanceDept(), buildMarketingDept(), buildOperationsDept() (+6 more)

### Community 93 - "CalcRightPanel.tsx"
Cohesion: 0.11
Nodes (10): DynamicFunnel, DynamicLeftPanel, DynamicRightPanel, DynamicSideBanners, jsonLd, metadata, BeforeRight(), CalcDoneDetail (+2 more)

### Community 94 - "SeoClustersClient.tsx"
Cohesion: 0.13
Nodes (12): dynamic, Brief, ClusterGroup, COMP_COLOR, COMP_LABEL, GROUP_LABELS, SeoClustersClient(), Stats (+4 more)

### Community 95 - "memory.ts"
Cohesion: 0.21
Nodes (19): ctxCache, dynamic, POST(), runtime, dynamic, POST(), runtime, addMessage() (+11 more)

### Community 96 - "app/services/page.tsx"
Cohesion: 0.12
Nodes (12): AUDIENCE, FAQ, FEATURES, FORMATS, PROBLEMS, SAAS_PLANS, metadata, SERVICE_LINKS (+4 more)

### Community 97 - "history/route.ts"
Cohesion: 0.43
Nodes (6): dynamic, ensureTable(), GET(), getClientId(), POST(), runtime

### Community 98 - "import-leads/crm.ts"
Cohesion: 0.16
Nodes (21): DELETE(), buildImportComment(), GET(), isAuthorized(), PATCH(), runtime, POST(), runtime (+13 more)

### Community 99 - "calculator/types.ts"
Cohesion: 0.11
Nodes (21): AI_STEPS, CalculatorForm(), CITY_CHIPS, initialFormData, inp(), SUBMIT_FALLBACK, CalculatorFormData, CalculatorResult (+13 more)

### Community 100 - "WbSellersClient.tsx"
Cohesion: 0.20
Nodes (8): dynamic, metadata, BADGE, parseExtra(), WbSellersClient(), КАТ_LABEL, Приоритет, РасширенныеДанные

### Community 101 - "NicheLeadPage.tsx"
Cohesion: 0.12
Nodes (9): metadata, metadata, metadata, metadata, metadata, metadata, NicheConfig, NicheLeadPage() (+1 more)

### Community 102 - "sales/data.ts"
Cohesion: 0.25
Nodes (13): maxDuration, POST(), runtime, buildFollowupQueue(), buildPipelineHealth(), buildSalesKPIs(), enhanceLead(), fetchAllSalesData() (+5 more)

### Community 103 - "telegram/webhook/route.ts"
Cohesion: 0.27
Nodes (14): ALLOWED, callAI(), dynamic, extractUrls(), MAIN_KEYBOARD, POST(), runEnrich(), runFunnelStatus() (+6 more)

### Community 104 - "tenders/page.tsx"
Cohesion: 0.24
Nodes (9): CompanyPanel(), DetailModal(), EnrichResult, PRIORITY_COLOR, rub(), Stats, STATUS_COLOR, TenderDashboard() (+1 more)

### Community 105 - "SupplierFinderUI.tsx"
Cohesion: 0.16
Nodes (11): metadata, EXAMPLES, PRICE_LABELS, QUALITY_LABELS, ScoreBreakdown, SearchQueries, SearchResult, Supplier (+3 more)

### Community 106 - "CreateWizard.tsx"
Cohesion: 0.13
Nodes (13): AI_MODULES, COUNTRIES, CreateWizard(), CURRENCIES, INDUSTRIES, PRESET_COLORS, STEPS, TIMEZONES (+5 more)

### Community 107 - "[id]/offer/route.ts"
Cohesion: 0.29
Nodes (10): dynamic, ensureAiColumns(), extractContacts(), fetchPage(), GET(), htmlToText(), POST(), runtime (+2 more)

### Community 108 - "placements/page.tsx"
Cohesion: 0.25
Nodes (10): ChannelType, DURATION_PRESETS, formatDate(), isExpired(), Placement, PlacementCard(), PlacementsPage(), POST_TYPE_COLORS (+2 more)

### Community 109 - "fix/route.ts"
Cohesion: 0.29
Nodes (10): checkExposedRoutes(), checkSecurityHeaders(), dynamic, fixCrmTestTenant(), fixOrphanTestLeads(), FixResult, maxDuration, POST() (+2 more)

### Community 111 - "ensureIntelligenceSchema"
Cohesion: 0.24
Nodes (13): dynamic, GET(), PATCH(), runtime, POST(), dynamic, GET(), runtime (+5 more)

### Community 112 - "AuditClient.tsx"
Cohesion: 0.20
Nodes (6): AuditClient(), AuditResult, Insight, MARKETPLACE_LABEL, STEPS, metadata

### Community 113 - "migrate-pg.cjs"
Cohesion: 0.29
Nodes (5): { Client }, envContent, envPath, { readFileSync }, { resolve }

### Community 114 - "ai-company/page.tsx"
Cohesion: 0.23
Nodes (11): AiOsDashboard(), AiCompanyPage(), dynamic, loadInitialReport(), metadata, GET(), maxDuration, runtime (+3 more)

### Community 115 - "SalesCompaniesClient.tsx"
Cohesion: 0.18
Nodes (9): dynamic, revalidate, Company, FILTER_OPTIONS, FilterKey, OfferResult, SalesCompaniesClient(), SCORE_COLOR (+1 more)

### Community 116 - "scripts"
Cohesion: 0.18
Nodes (10): name, private, scripts, build, dev, lint, seed:rates, start (+2 more)

### Community 117 - "CtoDashboard.tsx"
Cohesion: 0.40
Nodes (9): CheckRow(), CtoDashboard(), FixResult, fmtDate(), scoreBg(), scoreColor(), SectionCard(), statusBadge() (+1 more)

### Community 118 - "product-finder/page.tsx"
Cohesion: 0.28
Nodes (6): metadata, EXAMPLES, fmt(), Product, ProductFinderUI(), SearchResult

### Community 120 - "ImportLead"
Cohesion: 0.12
Nodes (12): IMPORTS_LABELS, Props, SCORE_COLORS, STATUS_LABELS, STATUS_STYLES, ACTION_ICON, AiSalesAgentClient(), Recommendation (+4 more)

### Community 121 - "articles/route.ts"
Cohesion: 0.27
Nodes (5): runtime, runtime, Article, ARTICLE_CATEGORIES, ARTICLES

### Community 122 - "outbound/page.tsx"
Cohesion: 0.13
Nodes (11): CHINA_MATCH_BADGE, First100Kpis, KpiData, NBA_LABELS, OFFER_LABELS, OutboundLead, PIPELINE_STAGES, Stage (+3 more)

### Community 123 - "presentation/page.tsx"
Cohesion: 0.22
Nodes (7): AUDIENCE, FEATURES, FORMATS, metadata, PROBLEMS, WHY, PrintButton()

### Community 124 - "finance/api.ts"
Cohesion: 0.06
Nodes (64): FinanceDashboardPage(), FinanceReportsPage(), dynamic, GET(), POST(), runtime, dynamic, PATCH() (+56 more)

### Community 125 - "analytics/index.ts"
Cohesion: 0.08
Nodes (30): KzAutoAccessoriesPage(), KzAutoPartsPage(), KzForm(), metadata, metadata, RuForm(), WhiteImportPage(), CountryDetectInit() (+22 more)

### Community 126 - "Weekly Priorities"
Cohesion: 0.15
Nodes (12): 1. Intel_facts: seed WB/Ozon/Kaspi commissions, 2. AI Product Analyzer → Lead capture, 3. CLAUDE.md structure (this task) ✅ In progress, 4. VK Pixel, 5. AI Sales Agent v1 — Architecture, 6. Intel_facts: CNY/USD auto-collector, Done This Week, P0 — Must ship this week (+4 more)

### Community 127 - "CHINABRIDGE — AI OPERATING SYSTEM"
Cohesion: 0.17
Nodes (11): AI AGENT RULES, CHINABRIDGE — AI OPERATING SYSTEM, CODE RULES, COMPLIANCE (IMMUTABLE), DATA RULES, graphify, INTELLIGENCE PIPELINE, MARKETING RULES (+3 more)

### Community 128 - "client/calculations/page.tsx"
Cohesion: 0.31
Nodes (7): GET(), runtime, CalculationsPage(), dynamic, fmtDate(), TRANSPORT_LABELS, getClientCalculations()

### Community 129 - "market-watch/route.ts"
Cohesion: 0.36
Nodes (8): dynamic, GET(), POST(), runtime, createMarketWatchItem(), ensureMarketWatchTable(), getMarketWatchItems(), updateMarketWatchPrice()

### Community 130 - "PersonalOfferWidget.tsx"
Cohesion: 0.18
Nodes (8): AnalysisSummary, AttackPlan, CockpitResult, ContactsFound, LOADING_STEPS, OfferOption, PersonalOfferWidget(), ScoreFactor

### Community 131 - "settings/marketing/page.tsx"
Cohesion: 0.22
Nodes (4): Message, QUICK_ACTIONS, SETTINGS_NAV, WELCOME

### Community 132 - "Marketplace.tsx"
Cohesion: 0.25
Nodes (6): CATEGORIES, Marketplace(), Module, MODULES, ModuleStatus, metadata

### Community 133 - "tenant-storage.ts"
Cohesion: 0.39
Nodes (7): CEO_DECISIONS_KEY(), CEO_HISTORY_KEY(), CEO_INBOX_KEY(), CEO_TASKS_KEY(), readTenantStorage(), tenantKey(), writeTenantStorage()

### Community 134 - "Russia Import Rules"
Cohesion: 0.17
Nodes (11): Air cargo, Common HS codes for our clients, Compliance Notes, Consolidated cargo (сборный груз), Customs Regime (White Scheme / Белая схема), Delivery Options, FCL container, Key Marketplaces (RU) (+3 more)

### Community 135 - "economics/route.ts"
Cohesion: 0.27
Nodes (7): checkAndIncrement(), getIp(), maxDuration, POST(), runtime, INTEL_MP_KEYS, SystemRates

### Community 136 - "KPIs by Category"
Cohesion: 0.17
Nodes (11): August (current month), KPIs by Category, Lead Generation, North Star, Product & Data, Q3 2026 — Current Quarter Goals, Q3 Milestones, Revenue & Clients (+3 more)

### Community 137 - "finance/settings/page.tsx"
Cohesion: 0.38
Nodes (5): dynamic, FinanceSettingsPage(), revalidate, FinanceSettingsForm(), FinanceSetting

### Community 138 - "getAllLeads"
Cohesion: 0.10
Nodes (26): analyzeLeadsWithAI(), GET(), isAuthorized(), LeadRecommendation, maxDuration, POST(), runtime, dynamic (+18 more)

### Community 139 - "contact-alert/route.ts"
Cohesion: 0.50
Nodes (4): dynamic, POST(), runtime, sendTelegram()

### Community 140 - "HhLeadsClient.tsx"
Cohesion: 0.22
Nodes (7): Filter, HhExtra, HhLeadsClient(), parseExtra(), КАТ_COLOR, КАТ_LABEL, dynamic

### Community 141 - "client-portal/api.ts"
Cohesion: 0.13
Nodes (22): dynamic, GET(), POST(), runtime, CITY_COUNTRIES, fmtDays(), getAIRec(), POST() (+14 more)

### Community 142 - "seed-db.ts"
Cohesion: 0.36
Nodes (8): n8nBase(), n8nFetch(), n8nKey(), normalize(), seedCreateRow(), seedListRows(), SeedTableKey, TABLE_IDS

### Community 143 - "crm/client.ts"
Cohesion: 0.17
Nodes (17): dynamic, LeadPage(), revalidate, DELETE(), dynamic, GET(), PATCH(), runtime (+9 more)

### Community 144 - "PartnersDashboard.tsx"
Cohesion: 0.29
Nodes (3): metadata, PartnersDashboard(), Stats

### Community 145 - "SignupForm.tsx"
Cohesion: 0.29
Nodes (4): metadata, COUNTRIES, EMPLOYEES, SignupForm()

### Community 146 - "isAdminOnly"
Cohesion: 0.24
Nodes (8): dynamic, POST(), runtime, WORKFLOWS, maxDuration, POST(), runtime, isAdminOnly()

### Community 147 - "vk-ads/sync/route.ts"
Cohesion: 0.24
Nodes (11): GET(), getToken(), leadToContact(), maxDuration, mtGet(), POST(), runSync(), runtime (+3 more)

### Community 148 - "multitenant/types.ts"
Cohesion: 0.07
Nodes (33): dynamic, COUNTRY_FLAG, fmtMoney(), PLAN_BADGE, PlatformDashboard(), STATUS_DOT, BillingClient(), Plan (+25 more)

### Community 149 - "rates/sync/route.ts"
Cohesion: 0.43
Nodes (6): dynamic, fetchCBRRates(), GET(), POST(), runtime, upsertRate()

### Community 150 - "wildberries-margin-calculator/page.tsx"
Cohesion: 0.22
Nodes (6): metadata, jsonLd, metadata, wbRates2026, Props, TelegramLeadCapture()

### Community 151 - "campaigns/page.tsx"
Cohesion: 0.28
Nodes (8): buildUtm(), Campaign, CampaignsPage(), CHANNEL_ICONS, CHANNELS, slugify(), STATUS_COLORS, STATUS_LABELS

### Community 152 - "getSession"
Cohesion: 0.14
Nodes (19): GET(), runtime, GET(), runtime, POST(), runtime, DashboardPage(), getCalcSubscription() (+11 more)

### Community 153 - "content-publish/route.ts"
Cohesion: 0.48
Nodes (6): ensureTable(), GET(), maxDuration, publishToTelegram(), runtime, sql

### Community 154 - "free/route.ts"
Cohesion: 0.38
Nodes (6): dynamic, escapeNonAscii(), FreeLeadInput, POST(), runtime, validate()

### Community 155 - "facts/route.ts"
Cohesion: 0.40
Nodes (5): dynamic, GET(), POST(), runtime, getFacts()

### Community 156 - "demo/documents/page.tsx"
Cohesion: 0.29
Nodes (5): DOC_TYPES, DOCS, STATUS_LABEL, STATUS_STYLE, TYPE_COLORS

### Community 157 - "approve/route.ts"
Cohesion: 0.31
Nodes (10): approvedText(), handoffToConsultant(), hotLeadText(), isHot(), LeadRow, maxDuration, PATCH(), POST() (+2 more)

### Community 158 - "AiSettings.tsx"
Cohesion: 0.33
Nodes (4): AiConfig, AiSettings(), PROVIDERS, metadata

### Community 159 - "BillingCenter.tsx"
Cohesion: 0.33
Nodes (4): BillingCenter(), INVOICES, PLANS, metadata

### Community 160 - "registrations/page.tsx"
Cohesion: 0.47
Nodes (5): Client, COUNTRY_FLAG, daysSince(), fmt(), RegistrationsPage()

### Community 161 - "database.ts"
Cohesion: 0.57
Nodes (6): checkConnection(), checkDatabase(), checkQueryPerf(), CntRow, countTable(), sql

### Community 162 - "schema.ts"
Cohesion: 0.12
Nodes (16): callAI(), CATEGORY_CHINA_MAP, maxDuration, POST(), runtime, cashFlow, financeExpenses, financeOrders (+8 more)

### Community 163 - "calculator.ts"
Cohesion: 0.16
Nodes (20): getAiAnalysis(), maxDuration, POST(), runtime, saveProductAnalysis(), sendTelegramAlert(), buildScenario(), calculateUnitEconomics() (+12 more)

### Community 164 - "lid-webhook/route.ts"
Cohesion: 0.07
Nodes (37): DRIP, dynamic, GET(), runtime, sendTg(), answerCallback(), AUTO_ACC_KB, AUTO_PARTS_KB (+29 more)

### Community 165 - "middleware.ts"
Cohesion: 0.48
Nodes (6): config, middleware(), verifyAdminToken(), verifyClientToken(), verifyPartnerToken(), verifyTenantSession()

### Community 166 - "vercel.json"
Cohesion: 0.29
Nodes (6): buildCommand, crons, devCommand, framework, installCommand, outputDirectory

### Community 167 - "dtQuery"
Cohesion: 0.50
Nodes (7): GET(), runtime, OrderDetailPage(), dtQuery(), getOrderById(), getOrderDocuments(), getOrderTracking()

### Community 168 - "VkIntentClient.tsx"
Cohesion: 0.20
Nodes (7): dynamic, Stats, Tier, TIER_ICON, TIER_STYLE, VkIntentClient(), VkLead

### Community 169 - "chat/page.tsx"
Cohesion: 0.25
Nodes (6): dynamic, revalidate, LeadContext, Message, PROMPTS, SalesChatClient()

### Community 170 - "tg-scraper/route.ts"
Cohesion: 0.22
Nodes (12): classifyIntent(), dynamic, GET(), INTENT_KEYWORDS, maxDuration, notifyManager(), parseMessages(), runtime (+4 more)

### Community 171 - "checks/crm.ts"
Cohesion: 0.73
Nodes (5): checkCrm(), checkLeadCrud(), checkLeadsExist(), checkTenantsTable(), sql

### Community 172 - "client/page.tsx"
Cohesion: 0.33
Nodes (4): DOCS, MSGS, ORDER, STATUSES

### Community 173 - "crm/page.tsx"
Cohesion: 0.33
Nodes (4): LEADS, PIPELINE, STATUS_LABELS, STATUS_STYLES

### Community 174 - "debug-secret.mjs"
Cohesion: 0.33
Nodes (5): envPath, eq, line, raw, v

### Community 175 - "tenant-auth.ts"
Cohesion: 0.22
Nodes (5): ROLE_PERMISSIONS, SUPER_ADMIN_COOKIE, TENANT_ADMIN_COOKIE, TenantSession, TenantRole

### Community 176 - "checks/api.ts"
Cohesion: 0.40
Nodes (5): AUTH, checkApi(), fetchRoute(), ROUTES, RouteSpec

### Community 177 - "store.ts"
Cohesion: 0.10
Nodes (31): dynamic, TenantPage(), GET(), maxDuration, runtime, DELETE(), GET(), isSuperAdmin() (+23 more)

### Community 178 - "white-import-wb-ozon/page.tsx"
Cohesion: 0.33
Nodes (4): FAQ, metadata, schema, STEPS

### Community 179 - "Architecture Decision Records (ADR)"
Cohesion: 0.20
Nodes (9): ADR-001: intel_facts as Single Source of Truth for Market Data, ADR-002: Chat Sessions Persisted in Neon DB, ADR-003: No void async() on Vercel Serverless, ADR-004: RU and KZ Always Separate Calculation Contexts, ADR-005: OpenRouter as LLM Provider with Fallback, ADR-006: lib/knowledge/index.ts for ChinaBridge Service Prices Only, ADR-007: Operations CEO AI Uses Real CRM Data, ADR-008: Finance Module Shows isDemo Warning When N8N Not Connected (+1 more)

### Community 181 - "trigger-enrich/route.ts"
Cohesion: 0.40
Nodes (5): dynamic, isAuthorized(), maxDuration, POST(), runtime

### Community 182 - "Intelligence Agent — Context"
Cohesion: 0.22
Nodes (8): API Endpoints, Collection Pipeline, Current Status (as of 2026-08-20), Data it owns, Intelligence Agent — Context, P0 Tasks, Purpose, Rules for Writing to intel_facts

### Community 183 - "wb-sellers/route.ts"
Cohesion: 0.60
Nodes (4): GET(), isAuthorized(), POST(), runtime

### Community 184 - "migrate/route.ts"
Cohesion: 0.40
Nodes (3): maxDuration, MIGRATION_STATEMENTS, runtime

### Community 186 - "Sales Agent System Prompt"
Cohesion: 0.22
Nodes (8): Before Writing Any Message, Compliance, Example Bad Opening (never do this), Example Good Opening (KZ), Message Structure, Output Format, Sales Agent System Prompt, Your Role

### Community 187 - "clients/route.ts"
Cohesion: 0.23
Nodes (12): DELETE(), dynamic, GET(), isAdmin(), runtime, GET(), PATCH(), runtime (+4 more)

### Community 189 - "marketing-ai/route.ts"
Cohesion: 0.50
Nodes (4): buildSystemPrompt(), maxDuration, POST(), runtime

### Community 191 - "migrate-outbound.mjs"
Cohesion: 0.50
Nodes (3): envContent, envPath, sql

### Community 192 - "init/route.ts"
Cohesion: 0.28
Nodes (7): dynamic, GET(), maxDuration, runtime, SEED_FACTS, SEED_SOURCES, CreateFactInput

### Community 193 - "demo/finance/page.tsx"
Cohesion: 0.40
Nodes (3): CATEGORIES, MONTHS, TRANSACTIONS

### Community 194 - "tripwire/route.ts"
Cohesion: 0.40
Nodes (5): dynamic, ensureTable(), POST(), runtime, tochkaAgent

### Community 195 - "Kazakhstan Import Rules"
Cohesion: 0.22
Nodes (8): ⚠️ CRITICAL COMPLIANCE RULE, Grey Scheme Mechanics (for internal context only — never for client-facing copy), Kaspi.kz — Key Platform, Kazakhstan Import Rules, Key Cities and Transit Times, Lead Source Context, Pricing Context, Proposal Template for KZ Clients

### Community 197 - "query-approved.mjs"
Cohesion: 0.40
Nodes (4): client, env, envFile, miLeads

### Community 201 - "CalcLeftPanel.tsx"
Cohesion: 0.28
Nodes (6): AfterLeft(), CALC_ITEMS, CalcDoneDetail, fmtRub(), verdictMeta(), EconomicsResult

### Community 205 - "Customer Success Agent — Context"
Cohesion: 0.25
Nodes (7): Compliance (same as sales), Customer Success Agent — Context, Data It Reads, Message Tone, Purpose, Triggers for Outreach, What to Track

### Community 206 - "demo-request/route.ts"
Cohesion: 0.67
Nodes (3): escapeNonAscii(), POST(), runtime

### Community 245 - "ai-funnel/route.ts"
Cohesion: 0.50
Nodes (4): dynamic, GET(), isAuthorized(), runtime

### Community 246 - "Sales Agent — Context"
Cohesion: 0.25
Nodes (7): How This Agent Works, Message Rules, Purpose, Qualification Scoring, Sales Agent — Context, System Prompt, Tools Available

### Community 247 - "SEO Agent — Context"
Cohesion: 0.25
Nodes (7): Content Generation Rules, intel_facts SEO Integration, Keyword Strategy, Pages Priority, Purpose, SEO Agent — Context, What NOT to Write

### Community 248 - "VkAdsDashboard.tsx"
Cohesion: 0.29
Nodes (5): AccountData, Campaign, LeadData, LeadForm, VkAdsDashboard()

### Community 250 - "ChinaBridge — Business Model"
Cohesion: 0.25
Nodes (7): ChinaBridge — Business Model, Competitive Position, Growth Levers (2026), Ideal Customer Profile (ICP), Revenue Streams, Unit Economics, What We Do

### Community 253 - "cbr/route.ts"
Cohesion: 0.33
Nodes (5): dynamic, fetchCbrRates(), maxDuration, POST(), runtime

### Community 254 - "Marketplace Commission Rules"
Cohesion: 0.29
Nodes (6): How intel_facts overrides hardcoded values, Kaspi.kz (Kazakhstan), Marketplace Commission Rules, Ozon, Wildberries (WB), Yandex Market

### Community 257 - "ChinaBridge — Business Knowledge"
Cohesion: 0.50
Nodes (3): ChinaBridge — Business Knowledge, Files in this directory, How to use this knowledge

### Community 262 - "lib/finance/types.ts"
Cohesion: 0.08
Nodes (34): dynamic, FinanceExpensesPage(), fmtDate(), revalidate, dynamic, FinanceOrdersPage(), fmtDate(), revalidate (+26 more)

### Community 263 - "currency/route.ts"
Cohesion: 0.33
Nodes (6): CbrValute, fetchCbrRates(), GET(), RatesResult, revalidate, runtime

### Community 264 - "import-consultant.ts"
Cohesion: 0.25
Nodes (9): AIConsultantPanel(), getInitialCTAs(), Message, Props, buildSystem(), CalcContext, DeliveryOption, detectAutoCategory() (+1 more)

### Community 265 - "queries/page.tsx"
Cohesion: 0.24
Nodes (9): CAT_LABELS, CategoryRow, fmt(), isJunk(), LeadsData, ProductRow, QueriesPage(), RecentRow (+1 more)

### Community 267 - "CeoReportWidget.tsx"
Cohesion: 0.20
Nodes (9): Alert, ALERT_STYLE, CeoReport, CeoReportWidget(), Insight, Priority, Stats, TREND_COLOR (+1 more)

### Community 268 - "telegram-capture/route.ts"
Cohesion: 0.83
Nodes (3): ensureTable(), notifyManager(), POST()

### Community 269 - "check-paid/route.ts"
Cohesion: 0.67
Nodes (3): dynamic, GET(), getClientId()

### Community 270 - "OnboardingSteps.tsx"
Cohesion: 0.40
Nodes (3): Props, Step, STEPS

### Community 273 - "reset-rate-limit.mjs"
Cohesion: 0.50
Nodes (3): dbUrl, envContent, sql

### Community 277 - "checks/telegram.ts"
Cohesion: 0.36
Nodes (8): checkBotApi(), checkBotCredentials(), checkTelegramBot(), sendTelegramMessage(), buildTelegramReport(), sectionLine(), sendCtoReport(), statusEmoji()

### Community 278 - "faq/page.tsx"
Cohesion: 0.29
Nodes (5): FAQ_CUSTOMS, FAQ_DELIVERY, FAQ_MARKETPLACE, FAQ_SUPPLIERS, metadata

### Community 279 - "update-rates.mjs"
Cohesion: 0.50
Nodes (3): envContent, envPath, sql

### Community 280 - "support-webhook/route.ts"
Cohesion: 0.40
Nodes (5): dynamic, GREETING_BUTTONS, POST(), runtime, sendViaSupport()

### Community 281 - "company-enrich/route.ts"
Cohesion: 0.27
Nodes (7): ContactInfo, dynamic, EgrulRaw, extractText(), fetchEgrul(), fetchRusprofile(), GET()

### Community 284 - "knowledge/chat/route.ts"
Cohesion: 0.50
Nodes (4): maxDuration, POST(), runtime, saveQuestion()

### Community 285 - "TochkaLinksSection.tsx"
Cohesion: 0.38
Nodes (6): fmtAmount(), fmtDate(), PaymentLink, STATUS_LABEL, STATUS_STYLE, TochkaLinksSection()

### Community 287 - "calc-verify/route.ts"
Cohesion: 0.67
Nodes (3): issueSubscription(), POST(), runtime

### Community 288 - "intake/route.ts"
Cohesion: 0.18
Nodes (15): HOT_SCORE, maxDuration, notifyManager(), POST(), runtime, buildFallback(), BusinessType, callOpenRouter() (+7 more)

### Community 290 - "create-tripwire/route.ts"
Cohesion: 0.40
Nodes (5): dynamic, ensureColumns(), POST(), runtime, tochkaAgent

### Community 291 - "ai-chat/page.tsx"
Cohesion: 0.25
Nodes (3): Message, MODELS, QUICK_PROMPTS

### Community 292 - "set-pro/route.ts"
Cohesion: 0.67
Nodes (3): dynamic, getClientId(), POST()

### Community 293 - "calculator-lead/route.ts"
Cohesion: 0.47
Nodes (5): dynamic, ensureTable(), fmtNum(), POST(), runtime

### Community 294 - "FAQSection.tsx"
Cohesion: 0.09
Nodes (16): FAQ, metadata, ROUTES, schema, FAQ, metadata, schema, FAQ (+8 more)

### Community 296 - "marketplace-rates/route.ts"
Cohesion: 0.33
Nodes (5): dynamic, PATCH(), runtime, upsertMpRate(), MARKETPLACES

### Community 302 - "seo.ts"
Cohesion: 0.60
Nodes (5): check404(), checkHomepageMeta(), checkRobots(), checkSeo(), checkSitemap()

### Community 308 - "trackGAEvent"
Cohesion: 0.07
Nodes (27): CATEGORIES, CITIES, ImportCategoryPage(), NotFound(), GAInner(), trackVkGoal(), VkPixel(), Window (+19 more)

### Community 309 - "funnel-report/route.ts"
Cohesion: 0.50
Nodes (4): dynamic, GET(), runtime, sendTg()

### Community 312 - "ai-chat/route.ts"
Cohesion: 0.18
Nodes (11): ensureTable(), GET(), GH_TOKEN(), ghListDir(), ghReadFile(), ghWriteFile(), maxDuration, POST() (+3 more)

### Community 314 - "product-finder/route.ts"
Cohesion: 0.47
Nodes (5): checkLimit(), getIp(), maxDuration, POST(), runtime

### Community 318 - "outbound/leads/route.ts"
Cohesion: 0.38
Nodes (5): GET(), mapLead(), maxDuration, queryLeads(), runtime

### Community 319 - "calc-funnel-tracker.ts"
Cohesion: 0.48
Nodes (6): CalcStep, getDevice(), getOrCreateAnonymousId(), getSessionId(), getSource(), trackCalcStep()

### Community 320 - "AdminNav.tsx"
Cohesion: 0.09
Nodes (15): Question, metadata, GROUP_COLOR, KwRow, TRAFFIC_COLOR, IntegrationsClient(), STATIC_INTEGRATIONS, VkStatus (+7 more)

### Community 323 - "payments.ts"
Cohesion: 0.48
Nodes (6): AUTH, checkCheckEndpoint(), checkCreateEndpoint(), checkPayments(), checkPaymentsTable(), checkTochkaEnv()

### Community 324 - "security.ts"
Cohesion: 0.48
Nodes (6): checkCronSecretSet(), checkJwtEnvSet(), checkProtected(), checkSecurity(), checkSecurityHeaders(), PROTECTED

### Community 325 - "rates/route.ts"
Cohesion: 0.50
Nodes (3): POST(), runtime, validateShippingRate()

### Community 326 - "china-blagoveshchensk/page.tsx"
Cohesion: 0.40
Nodes (3): FAQ, metadata, schema

### Community 327 - "runOutboundMigrations"
Cohesion: 0.26
Nodes (8): deriveCompanyId(), deriveOpportunityId(), maxDuration, POST(), runtime, GET(), runtime, runOutboundMigrations()

### Community 328 - "ai-providers.ts"
Cohesion: 0.47
Nodes (5): checkAiProviders(), checkOpenRouterKey(), MODELS, ModelSpec, testModel()

### Community 329 - "china-khabarovsk/page.tsx"
Cohesion: 0.40
Nodes (3): FAQ, metadata, schema

### Community 330 - "funnel/page.tsx"
Cohesion: 0.33
Nodes (6): convRate(), FunnelDashboard(), RealStats, STAGES, UPPER_ESTIMATES, VERTICAL_LABELS

### Community 332 - "notifications/route.ts"
Cohesion: 0.50
Nodes (4): GET(), maxDuration, runtime, generateNotifications()

### Community 335 - "app/layout.tsx"
Cohesion: 0.40
Nodes (3): inter, metadata, orgSchema

### Community 336 - "china-moscow/page.tsx"
Cohesion: 0.40
Nodes (3): FAQ, metadata, schema

### Community 338 - "supplier-search/page.tsx"
Cohesion: 0.40
Nodes (3): FAQ, metadata, schema

### Community 342 - "pay-supplier-china/page.tsx"
Cohesion: 0.40
Nodes (3): FAQ, metadata, schema

### Community 343 - "calc-funnel/page.tsx"
Cohesion: 0.67
Nodes (3): CalcFunnelPage(), FunnelData, pct()

### Community 344 - "calc-funnel/route.ts"
Cohesion: 0.67
Nodes (3): GET(), requireAdmin(), runtime

### Community 352 - "getLeads"
Cohesion: 0.14
Nodes (20): DashboardPage(), auth(), ensureTable(), GET(), POST(), dynamic, GET(), runtime (+12 more)

## Knowledge Gaps
- **1591 isolated node(s):** `runtime`, `dynamic`, `ExtractedPost`, `AnalyticsRow`, `CrmStats` (+1586 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **78 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AdminNav()` connect `AdminNav.tsx` to `FinanceDashboard.tsx`, `content/data.ts`, `analytics/data.ts`, `OperationsDashboard.tsx`, `strategy/data.ts`, `MarketingDashboard.tsx`, `lib/finance/types.ts`, `finance/settings/page.tsx`, `ContentPageClient.tsx`, `HhLeadsClient.tsx`, `generator.ts`, `crm/client.ts`, `LeadDetail.tsx`, `multitenant/types.ts`, `rate-engine/types.ts`, `campaigns/page.tsx`, `CeoDashboard.tsx`, `intelligence/page.tsx`, `SalesDashboard.tsx`, `registrations/page.tsx`, `deal-intelligence/index.ts`, `ai-chat/page.tsx`, `client-portal/types.ts`, `VkIntentClient.tsx`, `chat/page.tsx`, `market-intelligence/types.ts`, `MarketingPageClient.tsx`, `store.ts`, `TgMonitorClient.tsx`, `intake/page.tsx`, `DealIntelligenceClient.tsx`, `pricing/page.tsx`, `admin/calculations/page.tsx`, `ai-cto/db.ts`, `SalesDashboardClient.tsx`, `outreach-leads/page.tsx`, `SeoClustersClient.tsx`, `VkAdsDashboard.tsx`, `WbSellersClient.tsx`, `tenders/page.tsx`, `CreateWizard.tsx`, `ai-company/page.tsx`, `SalesCompaniesClient.tsx`, `ImportLead`, `outbound/page.tsx`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `createLead()` connect `createLead` to `getLeads`, `import-leads/crm.ts`, `calculator.ts`, `partner-portal/api.ts`, `tender-intelligence/route.ts`, `economics/route.ts`, `crm/client.ts`, `vk-ads/sync/route.ts`, `api/leads/route.ts`, `market-intelligence/db.ts`, `finance/api.ts`, `memory.ts`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `isAuthorized()` connect `isAuthorized` to `FinanceDashboard.tsx`, `content/data.ts`, `analytics/data.ts`, `OperationsDashboard.tsx`, `strategy/data.ts`, `MarketingDashboard.tsx`, `client-portal/api.ts`, `ceo/report/route.ts`, `createLead`, `vk-ads/sync/route.ts`, `SalesDashboard.tsx`, `payments/create/route.ts`, `product-finder/route.ts`, `partners/db.ts`, `setup/route.ts`, `notifications/route.ts`, `getLeads`, `sales/data.ts`, `ai-company/page.tsx`, `finance/api.ts`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **What connects `runtime`, `dynamic`, `ExtractedPost` to the rest of the system?**
  _1591 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `content/db.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.055964653902798235 - nodes in this community are weakly interconnected._
- **Should `marketing/db.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.05833905284831846 - nodes in this community are weakly interconnected._
- **Should `partner-portal/api.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.05368289637952559 - nodes in this community are weakly interconnected._