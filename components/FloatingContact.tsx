...
export default function FloatingContact() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // pulse animation to draw attention after 20s
    }, 20000);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/free", {
        ...
        body: JSON.stringify({ telegram: form.contact, name: form.name || undefined }),
      });
        ...
        <div className="fixed bottom-20 right-4 z-50 w-72 bg-[#0d1e36] border border-[#1e3a5f] rounded-2xl shadow-2xl shadow-black/60 overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
          ...
          {status === "ok" ? (
            <div className="px-4 py-5 text-center">
              ...
              <p className="text-[#8899aa] text-xs">Свяжемся с вами в Telegram в течение рабочего дня</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="px-4 py-4 space-y-3">
              ...
              {status === "error" && <p className="text-red-400 text-xs">Ошибка. Напишите напрямую в Telegram.</p>}
              <button
                type="submit"
              ...
              >
                {status === "loading" ? "Отправляем..." : "Оставить контакт"}
              </button>
              ...
              <div className="flex gap-2">
                <a
                  ...
                  onClick={() => { trackVkGoal("lead"); trackGAEvent("telegramBotClick", { source: "FLOATING_WIDGET" }); }}
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => { trackVkGoal("lead"); trackGAEvent("whatsappClick", { source: "FLOATING_WIDGET" }); }}
