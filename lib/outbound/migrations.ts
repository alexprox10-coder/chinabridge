...
export async function runOutboundMigrations(): Promise<void> {
...

  {
    ...
    const intakeColumnMigrations = [
      ...
      `ALTER TABLE outbound_lead_events ADD COLUMN IF NOT EXISTS supplier_exists BOOLEAN`,
      `ALTER TABLE outbound_lead_events ADD COLUMN IF NOT EXISTS supplier_source TEXT NOT NULL DEFAULT ''`,
