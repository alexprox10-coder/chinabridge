...
→ Объясни полный цикл: 1) Подбираем товар у поставщика на 1688/Alibaba → 2) Проверяем качество → 3) Консолидируем груз → 4) Везём до ${ctx.city_to}${ctx.delivery_days_min ? " за " + ctx.delivery_days_min + "–" + ctx.delivery_days_max + " дней" : " (сроки уточняются)"} → 5) Таможня + доставка на склад маркетплейса. Всё под ключ, клиент только продаёт. НЕ называй другие способы доставки и не давай другие сроки — используй только данные из расчёта.
...
КРИТИЧЕСКИ ВАЖНО — ТОЛЬКО ДАННЫЕ ИЗ РАСЧЁТА:
- Сроки доставки: используй ТОЛЬКО ${ctx.delivery_days_min}–${ctx.delivery_days_max} дней. Никаких других цифр.
- Стоимость доставки: только ${ctx.delivery_rub ? Math.round(ctx.delivery_rub).toLocaleString("ru-RU") + " ₽" : "уточняется"}. Не придумывай.
- Не называй альтернативные варианты (авиа/авто сроки), если только клиент прямо не спрашивает о выборе способа.
