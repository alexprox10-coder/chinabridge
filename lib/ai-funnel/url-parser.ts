...
export type ParseResult =
  ...
  | { ok: false; reason: 'unsupported_domain' | 'scrape_failed' | 'parse_failed' | 'no_key' };
...
async function scrape(url: string): Promise<string | null> {
  ...
  try {
    ...
    return ((d.data?.markdown ?? d.markdown ?? '') as string).slice(0, 8000) || null;
  } catch { return null; }
}

async function parseWithAI(platform: string, markdown: string): Promise<ParsedProduct | null> {
  if (!OPENROUTER_KEY) return null;
  const prompt = `Ты эксперт по закупкам из Китая. Из страницы товара (${platform}) извлеки данные и верни ТОЛЬКО JSON без markdown.
...
JSON формат:
{
  "product_name": "название товара (на русском, кратко)",
...
Правила:
- unit_price_cny: цена в юанях за единицу (только число, без валюты)
- weight_kg: вес единицы товара (если несколько вариантов — средний)
- confidence: high если нашёл цену и название, medium если только название, low если почти ничего`;
...
export async function parseProductUrl(url: string): Promise<ParseResult> {
  ...
  if (!FIRECRAWL_KEY)    return { ok: false, reason: 'no_key' };
